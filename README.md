...
面向墨西哥市场的用户画像与自然语言分析项目。系统以 `uid` 为主键，聚合 App 使用、行为事件、信用数据等多源信息，输出结构化画像、图表数据、Markdown 报告，并提供一个 `/api/orchestrator/*` 自然语言聊天助手用于调用画像工具、追踪任务上下文和管理本地长期记忆。

当前项目可以理解为两条并行能力：

- **画像主链路**：`App Profile`、`Behavior Profile`、`Credit Profile`、`Comprehensive Profile`。
- **Orchestrator Chat 链路**：自然语言对话、工具调用、session 管理、SQLite 长期记忆、Memory Inspector 调试/管理面板。

第一版长期记忆只服务 Orchestrator Chat，不影响 App/Behavior/Credit/Comprehensive 画像主链路输出。
...
3. 分别运行 `App / Behavior / Credit` 模块。
4. 汇总为 `Comprehensive` 综合画像。
5. 返回统一的 API 和前端展示结果。
...
│  ├─ repositories/                # 数据访问抽象
│  ├─ runtime_skills/              # App/Behavior/Credit/Comprehensive 画像模块
│  ├─ schemas/                     # API 响应契约
│  ├─ scripts/                     # 数据准备和辅助脚本
│  ├─ services/                    # 画像编排、Orchestrator Agent、Memory
│  ├─ static/                      # 前端 JSX/CSS/静态资源
│  ├─ ui/                          # 前端构建入口
│  └─ utils/
├─ data/                           # 本地数据目录
├─ data_acquisition_agent/         # 数据获取与知识库相关实验/运行模块
...
- `app/api/analyze.py`：`POST /api/analyze`
- `app/api/analyze_stream.py`：`POST /api/analyze-stream`
- `app/api/analyze_module.py`：单模块分析入口
- `app/api/trace.py`：trace 分析入口
...
其中 `app/runtime_skills/app_profile/` 和 `app/runtime_skills/credit_profile/` 已经比较接近分层结构：

- `contracts.py`：内部对象契约
- `data_access.py`：repository 到模块输入对象的适配
- `feature_builder.py`：特征提取
- `decision_engine.py`：规则判断
- `explainer.py`：LLM 解释增强
- `assembler.py`：页面/API 输出装配

### 3.5 `app/services/orchestrator_agent/`
...
- `tools/`：agent 可调用工具，例如查询数据、运行画像、记忆读写兼容工具。
...
flowchart LR
  ...
  ToolRegistry --> ProfileTools["run_profile / query_data / run_trace"]
...
- 左边是入口：浏览器前端、API 调用方。
- 中间分成两条主线：画像主链路和 Orchestrator Chat 链路。
- 画像主链路通过 `AnalysisOrchestrator` 调用四个 runtime skill。
...
| --- | --- | --- | --- | --- |
| API 层 | `app/api/*.py` | FastAPI、schemas、services | 前端、外部 API 调用方 | 接收请求、校验参数、返回响应 |
| 前端层 | `app/static/js/`、`app/ui/` | API helper、React-like 组件构建 | 浏览器 | Dashboard、NL Chat、Memory Inspector 展示 |
| 编排层 | `app/services/orchestrator.py` | repository、runtime skills、model client | `/api/analyze*`、agent tools | 调度画像模块并聚合结果 |
...
| 业务模块层 | `app/runtime_skills/*` | repository、prompts、model client、country packs | orchestrator、run_profile tool | App/Behavior/Credit/Comprehensive 画像 |
| 数据访问层 | `app/repositories/` | 本地文件或未来数据库 | 画像模块 | 读取数据，不做画像判断 |
| 模型层 | `app/core/model_client.py`、`app/core/providers/` | provider 配置、外部 LLM | explainer、agent loop | 统一 LLM 调用、fallback、结构化输出 |
| 配置层 | `app/core/config.py`、`.env` | 环境变量 | 全项目 | 读取模型、数据路径、memory flags |
| 测试评估层 | `tests/`、`scripts/` | pytest、临时 SQLite、TestClient | 开发者/CI | 单测、回归、离线评估、live E2E |
...
- `app/main.py` 注册 `/api/analyze`、`/api/analyze-stream`、`/api/analyze-module`、`/api/trace/{uid}`。
...
- `tools/` 中的工具由 `get_tool_registry()` 暴露给 agent loop。
...
- `app/static/js/services/api.js`：后端 API helper，包括 chat 和 memory 管理请求。
...
- `app/static/js/components/panels/`：画像结果面板，例如 App、Behavior、Credit、Comprehensive、Trace。
...
当前 Credit runtime 已逐步迁移到与 App Profile 类似的分层结构。
...
- `memories`：保存完整记忆记录和元数据。
- `memory_fts`：FTS5 虚拟表，用于 `content/tags/category` 检索。
...
## 6. Memory Inspector 使用说明

Memory Inspector 位于前端 `自然语言对话 / NL Chat` 页面里的“记忆”折叠面板，不是单独 Dashboard tab。
...
如果只是开发前端、Memory API、SQLite store 或大部分单测，可以使用 mock provider，避免真实 LLM 调用：
...
- 真实服务 E2E 如果要验证自然语言对话效果，需要可用的 `MODEL_MODE=gemini` 和 `GEMINI_API_KEY`。
...
python -m py_compile app/main.py app/api/orchestrator_routes.py
```
...
| 无模型开发 | `MODEL_MODE=mock uvicorn app.main:app --reload` | 适合 UI/API/store 开发 |

### 11.3 页面使用顺序

1. 打开首页 `/`。
2. 使用 Dashboard 的画像入口测试 `App / Behavior / Credit / Comprehensive` 展示。
3. 切到 `自然语言对话 / NL Chat`。
4. 输入自然语言任务，例如：
...
5. 展开 `记忆` 面板，查看本轮是否写入了有价值的 task/preference/project/reference 记忆。
6. 用 Memory Inspector 对错误记忆做归档、编辑或软删除。

## 12. 如何测试

### 12.1 Orchestrator / Memory 单测
...
全量测试可能受本地数据、模型配置、网络和历史实验模块影响。开发 Memory 或 NL Chat 时，优先跑上面的定向测试。
...
| 全项目回归 | `python -m pytest -q` | 视测试覆盖而定 | 视测试配置而定 |

### 12.7 Memory eval 如何扩充
...
当前更准确地说是“面向多智能体演进的 orchestrator + tools + runtime skills 架构”。画像模块、工具调用、session、memory、ACK、预算和上下文压缩已经具备 harness engineering 的雏形，但还不是完整的分布式多智能体平台。
...
1. 继续扩充 Memory eval dataset，把 smoke eval 扩成覆盖更多真实使用场景的评估集。
2. 完成 Memory Inspector 人工验收记录。
3. 为错误记忆处理补更细的管理体验，例如批量归档、导出、审计视图。
4. 在评估集显示 FTS 召回不足后，再进入 embedding / vector search。
5. 当记忆数量增长、冲突增多、需要归并时，再设计 consolidation / dreaming 后台任务。

当前项目在未来总系统中的定位是：

- 一个可被多智能体系统调用的国家画像引擎子系统。
- 一个正在逐步补齐 harness、memory、eval、debug 面板的本地智能体实验平台。
