"""Validate credit-profile structured output."""
...
REQUIRED_KEYS = ["uid", "status", "metrics", "tags", "evidence"]


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate credit-profile output JSON.")
    ...
    try:
        ...
        validated = CreditProfileStructuredResult.model_validate(structured_result).model_dump()
        dump_json({"ok": True, "validated": validated}, args.output)
        return 0
    except Exception as exc:  # pylint: disable=broad-except
        print(f"credit-profile validation failed: {exc}", file=sys.stderr)
        return 1
