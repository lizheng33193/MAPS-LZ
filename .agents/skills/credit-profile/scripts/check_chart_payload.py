"""Validate credit-profile chart payloads."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check credit-profile chart payloads.")
    ...
    try:
        ...
        if not charts and isinstance(payload, dict):
            charts = build_credit_charts(unwrap_structured_result(payload))
        ...
        print(f"credit chart check failed: {exc}", file=sys.stderr)
        return 1
