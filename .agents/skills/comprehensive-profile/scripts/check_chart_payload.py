"""Validate comprehensive-profile chart payloads."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check comprehensive-profile chart payloads.")
    ...
    try:
        ...
        if not charts and isinstance(payload, dict):
            charts = build_comprehensive_charts(unwrap_structured_result(payload))
        ...
        print(f"comprehensive chart check failed: {exc}", file=sys.stderr)
        return 1
