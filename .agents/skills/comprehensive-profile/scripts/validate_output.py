"""Validate comprehensive-profile structured output."""
...
REQUIRED_KEYS = ["uid", "status", "persona", "upstream_summaries", "metrics", "tags"]


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate comprehensive-profile output JSON.")
    ...
    try:
        ...
        validated = ComprehensiveProfileStructuredResult.model_validate(
            structured_result
        ).model_dump()
        dump_json({"ok": True, "validated": validated}, args.output)
        return 0
    except Exception as exc:  # pylint: disable=broad-except
        print(f"comprehensive-profile validation failed: {exc}", file=sys.stderr)
        return 1
